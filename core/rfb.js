...
export default class RFB extends EventTargetMixin {
    constructor(target, urlOrChannel, options) {
        ...
        this._clipboard = null;         // Clipboard handler object
        ...
        this._eventHandlers = {
            ...
            handleClipboardUpdate: this._handleClipboardUpdate.bind(this),
            handleClipboardAPIFail: this._handleClipboardAPIFail.bind(this),
        ...
        this._enabledAutomaticClipboard = false;
    }
    ...
    set viewOnly(viewOnly) {
        ...
        if (this._rfbConnectionState === "connecting" ||
            ...
            if (viewOnly) {
                ...
                this._keyboard.grab();
                if (this._enabledAutomaticClipboard) {
    ...
    get enabledAutomaticClipboard() {
        return this._enabledAutomaticClipboard;
    }

    set enabledAutomaticClipboard(enable) {
        if (!this._clipboard.available) {
            if (enable) {
                this.dispatchEvent(new CustomEvent("clipboardapifail"));
            }
            enable = false;
        }
        this._enabledAutomaticClipboard = enable;
        if (enable && !this._viewOnly) {
            this._clipboard.grab();
        } else if (!enable) {
            this._clipboard.ungrab();
        }
    ...
    _connect() {
        ...
        // Clipboard events
        this._clipboard.addEventListener("clipboardupdate", this._eventHandlers.handleClipboardUpdate);
        this._clipboard.addEventListener("clipboardapifail", this._eventHandlers.handleClipboardAPIFail);
    ...
    _disconnect() {
        ...
        this._clipboard.removeEventListener("clipboardupdate", this._eventHandlers.handleClipboardUpdate);
        this._clipboard.ungrab();
        this._sock.close();
        try {
    ...
    _handleClipboardUpdate(ev) {
        if (ev.detail.type !== "text") {
            return;
        }
    ...
    _handleServerCutText() {
        ...
        if (length >= 0) {
            ...
            if (this._enabledAutomaticClipboard) {
                this._clipboard.write("text", text);
            } else {
                this.dispatchEvent(new CustomEvent(
                    "clipboard",
                    { detail: { text: text } }));
            ...
            if (isCaps) {
                ...
                if (textData !== null) {
                    ...
                    if (this._enabledAutomaticClipboard) {
                        this._clipboard.write("text", textData);
                    } else {
                        this.dispatchEvent(new CustomEvent(
                            "clipboard",
